...
```
ssh -i ~/.ssh/openvpn openvpnas@32.192.36.65
```

```
for i in 01-vpc/ 02-sg/ 03-bastion/ 04-db/ 05-vpn/ 06-app-alb/ 07-backend/ 08-acm/ 09-web-alb/ 10-frontend/ 11-cdn/ ; do cd $i; terraform init ; cd .. ; done 
```

```
for i in 01-vpc/ 02-sg/ 03-bastion/ 04-db/ 05-vpn/ 06-app-alb/ 07-backend/ 08-acm/ 09-web-alb/ 10-frontend/ 11-cdn/ ; do cd $i; terraform plan; cd .. ; done 
```
```
for i in 01-vpc/ 02-sg/ 03-bastion/ 04-db/ 05-vpn/ ; do cd $i; terraform apply -auto-approve; cd .. ; done 
```
```
for i in  06-app-alb/ 07-backend/ 08-acm/ 09-web-alb/ 10-frontend/ 11-cdn/ ; do cd $i; terraform apply -auto-approve; cd .. ; done 
```
```
